...
UTXO_SATS=$(echo "$UTXO_VALUE * 100000000 / 1" | bc)
...
CHANGE_AMOUNT=$((UTXO_SATS - PAYMENT_AMOUNT - FEE_SATS))
check_cmd "Change calculation" "CHANGE_AMOUNT" "$CHANGE_AMOUNT"

# Convert amounts to BTC for createrawtransaction
PAYMENT_BTC=$(printf "%.8f" "$(echo "scale=8; $PAYMENT_AMOUNT / 100000000" | bc -l)")
CHANGE_BTC=$(printf "%.8f" "$(echo "scale=8; $CHANGE_AMOUNT / 100000000" | bc -l)")
...
# WRITE YOUR SOLUTION BELOW:
RAW_SEQUENCE=$(echo $DECODED_TX | jq -r '.vin[0].sequence')
if [ "$RAW_SEQUENCE" -lt 4294967295 ]; then
    VERIFY_RBF="true"
else
    VERIFY_RBF="false"
fi
...
CHANGE_BTC_VAL=$(bitcoin-cli -regtest decoderawtransaction $RAW_TX | jq -r '.vout[1].value')
CHANGE_SATS=$(echo "$CHANGE_BTC_VAL * 100000000 / 1" | bc)
...
CHILD_SEND_AMOUNT=$((CHANGE_SATS - CHILD_FEE_SATS))
check_cmd "Child amount calculation" "CHILD_SEND_AMOUNT" "$CHILD_SEND_AMOUNT"

# Convert to BTC
CHILD_SEND_BTC=$( printf "%.8f" $(echo "scale=8; $CHILD_SEND_AMOUNT / 100000000" | bc -l ))
...
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction $SECONDARY_TX | jq -r '.vout[0].value')
check_cmd "Secondary output value extraction" "SECONDARY_OUTPUT_VALUE" "$SECONDARY_OUTPUT_VALUE"

SECONDARY_SATS=$(echo "$SECONDARY_OUTPUT_VALUE * 100000000 / 1" | bc)
TIMELOCK_FEE=1000 # Use a simple fee of 1000 satoshis for this exercise
TIMELOCK_AMOUNT=$((SECONDARY_SATS - TIMELOCK_FEE))
